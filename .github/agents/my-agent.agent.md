...
- **Iconos:** Usa siempre `VectorIcon` (`src/components/icons/VectorIcon.tsx`) y/o `navIconFor()` para píldoras de navegación. No reinstales paquetes de íconos externos heredados; si falta un ícono, agrégalo al mapa SVG y reutiliza tokens (`currentColor`, `strokeWidth`).
